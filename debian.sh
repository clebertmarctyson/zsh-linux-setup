...
# Usage: chmod +x debian.sh && ./debian.sh
# ==============================================================================
set -euo pipefail

# ─────────────────────────────────────────────
# Guard: Debian/Ubuntu-based systems only
# ─────────────────────────────────────────────
if ! command -v apt >/dev/null; then
    echo "❌ This script is designed for Debian/Ubuntu-based systems only." >&2
    exit 1
fi

echo ""
echo "╔══════════════════════════════════════════╗"
echo "║         Zsh Power Setup — Debian         ║"
echo "╚══════════════════════════════════════════╝"
echo ""
...
if ! pipx list 2>/dev/null | grep -q thefuck; then
    echo "  → Installing thefuck via pipx..."
    pipx install thefuck
else
    echo "  ⏭ thefuck already installed via pipx, skipping."
fi
...
echo ""
echo "  Next steps:"
echo "  1. Run 'source ~/.zshrc' to reload your config"
